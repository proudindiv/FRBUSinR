...
<<>>=
write.csv(coeffs,"support/coeffs.csv")
@
...
eqs = eqs[[1]]
names = toupper(gsub(":.*$", "", eqs))
eqs = gsub("^.*:", "", eqs)
stdver = data.frame(eqs, row.names = names, stringsAsFactors = FALSE)
str(stdver)
stdver[c("CENG","PGDP"),"eqs"]
@

<<>>=
write.csv(eqs,"support/eqs.csv")
@
